...
<body style="background: url('https://wallpapercave.com/wp/bSbW3PB.jpg') no-repeat center center fixed;">
	<div class="container" style="background-color: white; padding-top: 20px;">
		...
		if(isset($_GET['tahun'])){
			...
			$tahun = $_GET['tahun'];
			if ($bulan == 'null') {
				$query = mysqli_query($koneksi,"SELECT DISTINCT(nomor_kelas), guru, bulan, tahun FROM jatah WHERE guru = '$username' AND tahun = $tahun");
			}else{
				$query = mysqli_query($koneksi,"SELECT DISTINCT(nomor_kelas), guru, bulan, tahun FROM jatah WHERE guru = '$username' AND tahun = $tahun AND bulan = '$bulan'");
			}
		}else{
			$query = mysqli_query($koneksi,"SELECT DISTINCT(nomor_kelas), guru, bulan, tahun FROM jatah WHERE guru = '$username'");
		}
		?>
		<div class="card mb-3">
						...
						<tbody>
							...
							?>
								<tr>
									...
									<td align="center"><a href="kelas.php?nomor_kelas=<?php echo $result['nomor_kelas']?>&bulan=<?php echo $result['bulan'] ?>&tahun=<?php echo $result['tahun'] ?>" class="btn btn-sm btn-warning"><i class="fa fa-arrow-right"></i></a></td>
								</tr>
								<?php 
									}
								?>
